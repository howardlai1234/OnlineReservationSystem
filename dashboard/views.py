...
def home(request):
    ...
    if 'username' in request.session:
        ...
        sql ='SELECT count(messageID) FROM Message WHERE receiverID="' + str(userid) + '"'
        cursor.execute(sql)
        row = cursor.fetchone()
        message_counter = row[0]

        #retrive meeting count from DB
        sql ='SELECT count(meetingID) FROM Meeting WHERE hostID="' + str(userid) + '" OR participantID="'  + str(userid) + '"'
        cursor.execute(sql)
        row = cursor.fetchone()
