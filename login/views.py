...
def home(request):
    ...
    # check whether it's valid:
        if form.is_valid():
            ...
            sql ='SELECT count(userID) FROM User WHERE username="' + form.cleaned_data['username'] + '" AND hashPW="' + form.cleaned_data['password'] + '"'
            cursor.execute(sql)
            row = cursor.fetchone()
            if (row[0] == 1):
                sql ='SELECT userID FROM User WHERE username="' + form.cleaned_data['username'] + '" AND hashPW="' + form.cleaned_data['password'] + '"'
                cursor.execute(sql)
                row = cursor.fetchone()
